...
class DeleteButtonWithConfirmation extends Component {
    ...
    handleDelete = ( event ) => {
        ...
        this.setState({ showDialog : false });
        const { dispatchCrudDelete, startUndoable, ressource, record, basePath, redirect, undoable } = this.props;
        if (undoable) {
            startUndoable(crudDelete(ressource, record.id, record, basePath, redirect));
        } else {
            dispatchCrudDelete(ressource, record.id, record, basePath, redirect);
        }
    };
        ...
        return (
            <Fragment>
                ...
                <Dialog fullWidth open={showDialog} onClose={this.handleCloseClick} aria-label="Are you sure?">
                    ...
                    <DialogContent>
                        <div>
                            Cette suppression sera définitive.                        </div>
                    </DialogContent>
                    <DialogActions>
